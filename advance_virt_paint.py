...
while True:
    ...
    cv2.putText(frame, text="ERASER", org=(1132, 60), fontFace=cv2.FONT_HERSHEY_COMPLEX, fontScale=1, color=(0, 0, 0),
                thickness=2)
    cv2.putText(frame, text="Circle", org=(997, 60), fontFace=cv2.FONT_HERSHEY_COMPLEX_SMALL, fontScale=1,
                color=(0, 0, 0), thickness=1)
    cv2.putText(frame, text="Rectangle", org=(818, 60), fontFace=cv2.FONT_HERSHEY_COMPLEX_SMALL, fontScale=1,
                color=(0, 0, 0), thickness=1)

    # 2. find hand landmarks
    frame = detector.find_hands(frame)  # it will detect hands from frames
    lm_list = detector.find_position(
        frame)  # lmlist is landmark list, the position coordinates of detected hand from frames is stored as list in lmlist
    # print(lmlist)

    if len(lm_list) != 0:
        x1, y1 = lm_list[8][1:]  # selecting the coordinates of  tip of index finger
        ...
        fingers = detector.fingers_up()  # to detect which finger is up
        ...
        if fingers[1] and fingers[2]:  # where fingers[1] means index finger and fingers[2] means middle finger
            ...
            cv2.rectangle(frame, (x1, y1), (x2, y2), draw_color,
                          -3)  # drawing a rectangle at the finger tips to indicate chosen drawing colors
        ...
        if fingers[1] and not fingers[2]:
            ...
            if draw_color == (0, 0, 0):
                cv2.line(frame, (xp, yp), (x1, y1), draw_color, thickness=eraser_size)
